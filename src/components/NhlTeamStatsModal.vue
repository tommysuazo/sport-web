          ...
          <table class="games-table">
            ...
            <tbody>
              <tr v-for="game in visibleGames" :key="game.id">
                ...
                <td>{{ game.rival }}</td>
                <td class="score-cell">
                  <strong>{{ game.goals.team ?? '-' }}</strong>
                  <span class="score-separator">-</span>
                  <span>{{ game.goals.opponent ?? '-' }}</span>
                </td>
                <td>
                ...
                <td>
                  <span class="metric metric--neutral">{{ formatNumber(game.shots) }}</span>
                </td>
              </tr>
            </tbody>
          </table>
...
const TEAM_GAMES_URL = (teamId) => buildNhlApiUrl(`games?team=${encodeURIComponent(teamId)}`);
...
const teamRecord = computed(() => {
  ...
  parsedGames.value.forEach((game) => {
    if (game.goals.team === null || game.goals.opponent === null) return;
    if (game.goals.team > game.goals.opponent) record.wins += 1;
    else if (game.goals.team < game.goals.opponent) record.losses += 1;
    else record.ot += 1;
  });
  return record;
});
...
function transformGame(game, teamId) {
  ...
  const teamGoals = parseNumeric(teamEntry.goals);
  const opponentGoals = parseNumeric(opponentEntry?.goals);
  const combinedGoals = teamGoals !== null && opponentGoals !== null ? teamGoals + opponentGoals : null;

  const market = resolveMarketObject(game.market);
  const handicapValue = computeHandicapValue(market, teamId);
  const handicapOutcome = evaluateHandicap(handicapValue?.line, teamGoals, opponentGoals);

  const totalsValue = parseNumeric(market?.total_points);
  const totalsOutcome = evaluateTotals(totalsValue, combinedGoals);

  const soloValue = determineSoloPoints(market, isAway);
  const soloOutcome = evaluateSolo(soloValue, teamGoals);
  ...
  return {
    ...
    shots: parseNumeric(teamEntry.shots),
  };
}
...
function computeHandicapValue(market, teamId) {
  ...
  return { line: normalizedLine, display, isFavorite };
}

function evaluateHandicap(line, teamGoals, opponentGoals) {
  if (line === null || teamGoals === null || opponentGoals === null) return 'metric--neutral';
...
function evaluateTotals(totalLine, combinedGoals) {
  if (totalLine === null || combinedGoals === null) return 'metric--neutral';
  if (totalLine < combinedGoals) return 'metric--positive';
  if (totalLine > combinedGoals) return 'metric--negative';
  return 'metric--neutral';
}
...
function evaluateSolo(soloLine, teamGoals) {
  if (soloLine === null || teamGoals === null) return 'metric--neutral';
  if (soloLine < teamGoals) return 'metric--positive';
  if (soloLine > teamGoals) return 'metric--negative';
  return 'metric--neutral';
}
